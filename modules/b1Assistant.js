                    ...
                    if (validate) {
                        ...
                        if (order == null) {
                            ...
                            while (!orderData && index < response.data.length) {
                                if (order.replace(/ /g, "").toUpperCase() === response.data[index].U_DescPedido.replace(/ /g, "").toUpperCase()) {
                                    orderData = response.data[index].U_DescPedido;
                                }
                            }
                            ...
                            if (!orderData) {
                                ...
                                TELEGRAM.PostRecurringOrders(sendJSON, function(err, orderResponse) {
                                    if (err) {
                                        console.error(err)
                                        speechOutput = "Hubo un problema en la comunicación con Telegram. Porfavor intentelo de nuevo " + err.message
                                    } else {

                                        speechOutput = orderResponse.message;
