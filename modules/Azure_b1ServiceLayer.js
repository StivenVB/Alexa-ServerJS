...
module.exports = {
    ...
    GetSalesInfoYear: function (year, callback) {
        return (GetSalesInfoYear(year, callback))
    },
...
function ServiceLayerRequest(isReport, method, endpoint, body, callback) {
    ...
    getCookiesCache().then(function (cookies) {
            var options = {
                    url: "https://servicesunit2.abilitysap.com.co" + ":" + 50000 
                        + (isReport ? "/b1s/v1/sml.svc" : "/b1s/v1") 
...
let Connect = function () {
    return new Promise(function (resolve, reject) {
        ...
        var uri = "https://servicesunit2.abilitysap.com.co" + ":" + "50000" + "/b1s/v1" + "/Login"

        //B1 Login Credentials
        var data = {
            UserName: "ABILITYSAP\\css-egranada",
            Password: "T*o6HoBibo6runUF+lsW",
            CompanyDB: "CONSENSUS_SAS",
            Language: "25"
        };
        ...
        console.log("Connecting to SL on " + uri + " - " + JSON.stringify(data));
...
function GetSalesInfo(year, quarter, callback) {
    ...
    //var oDataEndpoint = "/BusinessPartners('112333333')"
    ...
    ServiceLayerRequest(true, "GET", oDataEndpoint, null, function (error, response, body) {
        ...
        if (!error && response.statusCode == 200) {
            console.log("ENTRO AL 200"+ body);
            body = JSON.parse(body);
            
            callback(null, body);
        } else {
            console.log(error);
            console.log("BODY ENTRO", body);
            callback(error);
        }
    });
}

function GetSalesInfoYear(year, callback) {
    var select = ""
    ...
    // Set Odata query
    oDataEndpoint += "?$apply=groupby((PostingYear, BusinessPartnerCurrency)," +
        "aggregate($count as ItemCode_COUNT, NetSalesAmountLC with sum as NetSalesAmountLC_SUM))" +
        "&$filter=PostingYear eq " + quotes(year) + "and BusinessPartnerCurrency eq \'COP\'";
...
function PostSalesOrder(item, qty, item2, callback) {
    ...
    var bodySO = {
        CardCode: process.env.DEFAULT_BP || 'CL00000124',
